...
const outputDir = path.join(__dirname, '../public/data/nbla');
...
async function downloadAndConvert() {
  console.log('\n📖 Descargando NBLA de GitHub...\n');
  const response = await fetch('https://raw.githubusercontent.com/mrk214/bible-data-es-spa/main/data/es___spa___spa/NBLA_vid_103.json');
  if (!response.ok) throw new Error(`Failed to fetch NBLA: ${response.statusText}`);
  ...
  for (const book of raw.books) {
    ...
    if (!meta) { console.warn(`  ⚠️  No meta for book ID: ${bookId}`); continue; }
        ...
        $('*').each((_, el) => {
          if ($(el).hasClass('heading')) {
            ...
            if (!isNaN(verseNum) && verseText) {
              ...
              if (existing) existing.text += ' ' + verseText;
              else {
  ...
  console.log(`\n✅ Listo. ${count}/66 libros generados en public/data/nbla/\n`);
}
